...
service.interceptors.response.use(
  response => {
    ...
    let url = config.url
    // 特殊接口处理 
    if (url.indexOf('/Base/DataSource/Actions/Test') > -1 ||url.indexOf('/dev/api/extend/projectDetail/Actions/Export') == 0 || config.url.indexOf('/api/extend/projectDetail/templateDownload') > -1||(url.indexOf('Model') > -1 && url.indexOf('Config') > -1 )) return res
    if (res.code !== 200) {
      message({
