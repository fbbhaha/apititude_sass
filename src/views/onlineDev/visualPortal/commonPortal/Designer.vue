          ...
          <div class="color-list">
            <template v-for="(item, index) in colorList">
              ...
              >
                <i
                  class="el-icon-check"
                  v-if="item.className === styleForm.themeColor"
                ></i>
              </el-tag>
            </template>
          </div>
...
// import { customComponents } from "@/lib/lib.umd.min.js";
...
export default {
  ...
  computed: {
    ...
    layoutStyle() {
      ...
      if (this.config.layoutBg) {
        obj.backgroundImage = `url(${
          this.define.comUrl
        }/api/visualdev/PortalTheme/file/${this.config.layoutBg})`;
      }
    ...
    listStyle() {
      return index => {
        let height = "";
        if (this.leftComponents.length === 3) {
          height = "calc(25% - 25px)";
        } else {
          height = index ? "calc(25% - 25px)" : "calc(41% - 25px)";
        }
        return {
          height: height,
          marginTop: "10px"
  ...
  mounted() {
    ...
    if (typeof this.conf === "object" && JSON.stringify(this.conf) !== "{}") {
      ...
      this.config = Object.assign(this.config, this.initStyleForm);
    }
    if (!this.config.layoutBg) {
  ...
  methods: {
    ...
    cardStyleChange(e) {
      if (e === "default") {
        this.styleForm.themeColor = "";
      } else {
        this.styleForm.themeColor = "blue";
      }
    },
    changeColor(item) {
      if (item.className === this.styleForm.themeColor) {
        return;
      }
      this.styleForm.themeColor = item.className;
    },
    showStyleForm() {
      if (this.initStyleForm.themeColor !== "") {
