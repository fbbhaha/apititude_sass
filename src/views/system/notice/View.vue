<template>
  <el-dialog title="查看公告" :close-on-press-escape="false" :close-on-click-modal="false"
    :visible.sync="visible" class="CCIT-dialog CCIT-dialog_center CCIT-dialog-notice" lock-scroll
    width="80%">
    <div class="notice-wrapper" v-loading="loading">
      <h1 class="title">{{dataForm.title}}</h1>
      <div class="info">
        <span>{{ccit.dateFormat(dataForm.lastModifyTime)}}</span><span>{{dataForm.creatorUser}}</span>
      </div>
