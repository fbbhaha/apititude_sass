  ...
  methods: {
    ...
    initData() {
      ...
      FlowDelegateList(query).then(res => {
        ...
        let currTime = this.ccit.toDate(new Date())
        for (let i = 0; i < this.list.length; i++) {
          let e = this.list[i];
          let startTime = this.ccit.toDate(e.startTime);
          let endTime = this.ccit.toDate(e.endTime);
    ...
    formatter(row, column) {
      return this.ccit.dateFormat(row, column)
    },
    // 新增 / 修改
