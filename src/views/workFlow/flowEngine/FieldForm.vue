  ...
  <div class="fieldFrom">
    ...
    <el-table :data="drawingList" size='mini'>
      ...
      <el-table-column prop="filedId" label="控件类型">
        <template slot-scope="scope">
          <el-select v-model="scope.row.ccitKey" placeholder="选择控件类型" clearable>
            <el-option-group v-for="group in componentList" :key="group.id"
              :label="group.fullName+'【'+group.children.length+'】'">
              <el-option v-for="item in group.children" :key="item.__config__.ccitKey"
                :label="item.__config__.label" :value="item.__config__.ccitKey">
              </el-option>
            </el-option-group>
          </el-select>
        </template>
...
const ignoreList = ['divider', 'CCITText', 'button', 'table', 'relationFormAttr', 'popupAttr', 'calculate']
export default {
  props: ['conf', 'enCode'],
  data() {
    return {
  ...
  methods: {
    ...
    */
    ...
    getComponentList() {
      const realInputComponents = inputComponents.filter(o => !ignoreList.includes(o.__config__.ccitKey))
      const realSelectComponents = selectComponents.filter(o => !ignoreList.includes(o.__config__.ccitKey))
      this.componentList = [
        {
          id: '1',
          fullName: '基础控件',
