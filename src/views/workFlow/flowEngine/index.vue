<template>
  <div class="CCIT-common-layout">
        ...
        <CCIT-table v-loading="listLoading" :data="list">
          ...
          <el-table-column prop="formType" label="表单类型" width="100">
            <template slot-scope="scope">
              <span>{{ scope.row.formType == 1? "系统表单" : (scope.row.formType == 2 ?"自定义表单" : "外部表单") }}</span>
            </template>
          </el-table-column>
    ...
    <previewDialog :visible.sync="previewDialogVisible" :id="currRow.id" type="flow"
      @previewPc="previewPc" />
    <el-dialog title="新建表单" :visible.sync="dialogVisible" class="CCIT-dialog CCIT-dialog_center"
      ...
      <div class="add-main">
        ...
        <div class="add-item add-item-external" @click="addFlow(3)">
          <i class="add-icon el-icon-document"></i>
          <div class="add-txt">
            <p class="add-title">外部表单</p>
            <p class="add-desc">关联外部系统表单</p>
          </div>
        </div>
      </div>
    </el-dialog>
