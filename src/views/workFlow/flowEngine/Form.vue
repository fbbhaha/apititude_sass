<template>
  ...
  >
    <div class="CCIT-full-dialog-header">
      ...
      <div class="options">
        <el-button @click="prev" :disabled="activeStep <= 0">{{
          $t("common.prev")
        }}</el-button>
        <el-button @click="next" :disabled="activeStep >= 2 || loading"
          >{{ $t("common.next") }}
        </el-button>
        <el-button
          type="primary"
          ...
          >
            ...
            <el-form-item label="流程类型" prop="type">
              <el-select
                ...
                placeholder="选择类型"
                @change="typeChange"
                :disabled="!!dataForm.id || dataForm.formType == 2"
              >
              ...
              <el-form-item label="流程图标" prop="icon">
                <el-row type="flex">
                  <div style="flex: 1; margin-right: 10px">
                    ...
                    >
                      <el-button slot="append" @click="openIconBox()"
                        >选择</el-button
                      >
                    </el-input>
                  </div>
                  <el-color-picker
                    v-model="dataForm.iconBackground"
                    title="图标背景色"
                    :predefine="[
                      '#008cff',
                      '#35b8e0',
            ...
            <ccit-form-tip-item
              v-if="dataForm.formType === 3"
              label="外部地址"
              prop="sysUrl"
              tip-label="vue项目请输入地址+路由 h5项目请输入地址+页面路径"
            >
              <el-input v-model="dataForm.sysUrl" placeholder="系统地址">
              </el-input>
            </ccit-form-tip-item>
            ...
            <template v-if="dataForm.formType == 2">
              ...
              >
                ...
                <el-table-column prop="table" label="表名">
                  <template slot-scope="scope">
                    <el-tooltip
                      :content="scope.row.tableName || scope.row.table"
                      placement="top"
                    >
...
export default {
  ...
  data() {
    var checkUrl = (rule, value, callback) => {
      if (!value) {
        return callback(new Error("系统地址不能为空"));
      }
      var reg = new RegExp(
    ...
    return {
      ...
      dataRule: {
        ...
        sysUrl: [{ required: true, validator: checkUrl, trigger: "blur" }],
        formUrl: [
  ...
  methods: {
    init(categoryList, id, formType) {
      ...
      this.$nextTick(() => {
        if (this.dataForm.id) {
          FlowEngineInfo(this.dataForm.id)
            .then((res) => {
              this.dataForm = res.data;
              this.formType =
                this.dataForm.formType == 1
                  ? "系统表单"
                  : this.dataForm.formType == 2
                  ? "自定义表单"
                  : "外部表单";
              this.dataForm.flowTemplateJson &&
                (this.flowTemplateJson = JSON.parse(
                  this.dataForm.flowTemplateJson
                ));
              this.dataForm.formData &&
                (this.formData = JSON.parse(this.dataForm.formData));
              this.tables =
                (this.dataForm.tables && JSON.parse(this.dataForm.tables)) ||
                [];
              this.defaultTable =
                (this.dataForm.tables && JSON.parse(this.dataForm.tables)) ||
                [];
              this.updateFields();
              this.$set(this.dataForm, "dynamicForm", 1);
              this.loading = false;
          ...
          this.formType =
            this.dataForm.formType == 1
              ...
              ? "自定义表单"
              : "外部表单";
          this.dataForm.dynamicForm = 1;
          this.loading = false;
        }
      });
    },
    dataFormSubmit() {
      this.$refs["processDesign"]
        .getData()
        .then((res) => {
          this.btnLoading = true;
          this.flowTemplateJson = res.formData;
          this.dataForm.flowTemplateJson = JSON.stringify(
        ...
        this.$refs["dataForm"].validate((valid) => {
          if (valid) {
            ...
            if (!this.tables.length) {
              ...
              this.$store.commit(
                "generator/UPDATE_FORMITEM_LIST",
