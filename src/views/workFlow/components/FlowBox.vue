    ...
    <div class="CCIT-preview-main flow-form-main">
      ...
      <el-tabs class="CCIT-el_tabs" v-model="activeTab">
        <el-tab-pane label="表单信息" v-loading="loading">
          <template v-if="setting.formType == 3">
            <iframe
              ref="iframe"
      ...
      <el-dialog
        :title="eventType === 'audit' ? '审批通过' : '审批拒绝'"
...
export default {
  ...
  methods: {
    ...
    init(data) {
      ...
      window.onmessage = function (e) {
        if (e.data) {
          ...
          switch (method) {
            case "goBack":
              _this.goBack(e.data.data);
              break;
            case "getData":
              if (!e.data.valid) return;
              _this.eventReceiver(e.data.dataForm, e.data.eventType);
              break;
            case "handleRequest":
              _this.formData = Object.assign(_this.formData, e.data.dataForm);
              _this.handleRequest();
            default:
              break;
          }
      ...
      FlowEngineInfo(data.flowId)
        .then((res) => {
          ...
          if (data.formType == 1) {
            ...
            this.sysUrl = res.data.sysUrl + '?token=' + getToken()
                  ...
                  iframeWin.postMessage(
                    {
                      ...
                      data: data,
                      formKey: +new Date(),
                      userInfo: _this.$store.getters.userInfo,
                      token: _this.$store.getters.token,
                    },
                    "*"
                  );
                  iframeWin.postMessage(_this.formData, "*");
                });
              } else {
                // iframe的加载完成函数
      ...
      FlowBeforeInfo(data.id, {
        ...
        .then((res) => {
          ...
          if (data.formType == 1) {
            ...
            this.sysUrl = res.data.flowTaskInfo.sysUrl + '?token=' + getToken()
            this.iframeVisible = true;
            let _this = this;
            this.$nextTick(() => {
              const mapFrame = this.$refs["iframe"];
    ...
    saveAudit() {
      ...
      SaveAudit(this.setting.taskId, query)
        .then((res) => {
          this.$message({
            message: res.msg,
            type: "success",
    ...
    submitCandidate(data) {
      this.formData.candidateList = data;
      if (this.setting.formType == 3) {
        let _this = this;
        const iframeWin = this.$refs["iframe"].contentWindow;
        iframeWin.postMessage(
    ...
    handleApproval() {
      this.$refs["candidateForm"].validate((valid) => {
        if (valid) {
          ...
          approvalMethod(this.setting.taskId, query)
            .then((res) => {
              this.$message({
                type: "success",
                message: res.msg,
