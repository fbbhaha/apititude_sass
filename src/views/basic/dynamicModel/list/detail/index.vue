  ...
  <div>
    ...
    <template v-if="formData.popupType==='drawer'">
      <el-drawer title="详情" :visible.sync="visible" :wrapperClosable="false"
        :size='formData.drawerWidth' append-to-body class="CCIT-common-drawer">
        <div class="CCIT-flex-main">
  ...
  methods: {
    ...
    handleAttrList(list) {
      ...
      for (let i = 0; i < realList.length; i++) {
        ...
        let modelId = '', id = "", field = "", ccitKey = '', activeItem = {}
        let prop = item.relationField.split('_ccitTable_')[0]
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            if (prop === list[i].__vModel__) {
              ccitKey = list[i].__config__.ccitKey
              modelId = list[i].__config__.ccitKey === 'relationForm' ? list[i].modelId : list[i].interfaceId
              id = list[i].__config__.defaultValue
              field = list[i].__config__.tableName ? list[i].__vModel__ + '_ccitTable_' + list[i].__config__.tableName + (list[i].__config__.isSubTable ? '0' : "1") : list[i].__vModel__
              activeItem = list[i]
              break
            }
    ...
    fillFormData(form, data) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          let item = list[i]
          if (item.__vModel__) {
            if (item.__config__.ccitKey === 'relationForm' || item.__config__.ccitKey === 'popupSelect') {
          ...
          if (['relationFormAttr', 'popupAttr'].includes(item.__config__.ccitKey)) relationFormAttrList.push(item)
