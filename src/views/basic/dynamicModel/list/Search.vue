    ...
    <el-form @submit.native.prevent>
      <el-col :span="6" v-for="(item, index) in searchList" :key="index">
        <el-form-item :label="item.__config__.label" v-show="index<3 || (index>=3 && showAll)">
          ...
          <template v-else-if="useDateList.indexOf(item.__config__.ccitKey)>-1">
          ...
          <template v-else>
            <template
              v-if="item.__config__.ccitKey==='numInput'||item.__config__.ccitKey==='calculate'">
            ...
            <template
              v-if="item.__config__.ccitKey==='time'|| item.__config__.ccitKey==='timeRange'">
  ...
  methods: {
    buildOptions(componentList) {
      componentList.forEach(cur => {
        ...
        if (config.ccitKey === 'cascader') cur.props.props.multiple = false
