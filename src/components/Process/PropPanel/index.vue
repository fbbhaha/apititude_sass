...
<template>
  <el-drawer size="550px" class="drawer CCIT-common-drawer" :visible.sync="visible" @close="cancel"
      ...
      <template v-for="(item, index) in pconditions">
        <el-row class="condition-list" :key="index">
          ...
          <el-col :span="7" class="filedValue">
            ...
            <template v-else-if="['date','createTime', 'modifyTime'].includes(item.ccitKey)">
            ...
            <template v-else-if="['comSelect','currOrganize'].includes(item.ccitKey)">
              <comSelect v-model="item.filedValue" placeholder="请选择" clearable
                @change="onConditionListChange(arguments,item)" />
            </template>
            <template v-else-if="['depSelect'].includes(item.ccitKey)">
              <depSelect v-model="item.filedValue" placeholder="请选择" clearable
                @change="onConditionObjChange(arguments,item)" />
            </template>
            <template v-else-if="['userSelect','createUser','modifyUser'].includes(item.ccitKey)">
              <userSelect v-model="item.filedValue" placeholder="请选择" hasSys clearable
                @change="onConditionObjChange(arguments,item)" />
            </template>
            <template v-else-if="['posSelect','currPosition'].includes(item.ccitKey)">
              <posSelect v-model="item.filedValue" placeholder="请选择" clearable
                @change="onConditionObjChange(arguments,item)" />
            </template>
            <template v-else-if="item.ccitKey==='address'">
              <CCITAddress v-model="item.filedValue" placeholder="请选择" :level="item.level" clearable
                @change="onConditionListChange(arguments,item)" />
            </template>
            <template v-else>
          ...
          <el-scrollbar class="config-scrollbar">
            <el-form label-position="top" :model="subFlowForm" class="pd-10">
              ...
              <el-form-item label="子流程表单">
                <CCIT-TreeSelect :options="flowOptions" v-model="subFlowForm.flowId"
      ...
      <el-tabs style="height:100%;">
            ...
            <el-form label-position="top" class="pd-10">
              <el-form-item label="操作设置">
                ...
                <div class="per-cell" v-if="startForm.hasPrintBtn">
                  ...
                  <CCIT-TreeSelect :options="printTplList" v-model="startForm.printId"
                    placeholder="请选择打印模板" lastLevel clearable></CCIT-TreeSelect>
                </div>
              </el-form-item>
            </el-form>
        ...
        <el-tab-pane label="表单权限">
          <div class="form-auth-table">
            <el-table :data="getFormOperates()" class="CCIT-common-table" size="mini" height="100%">
            ...
            <el-form label-position="top" class="pd-10">
              <el-form-item label="操作设置">
                ...
                <div class="per-cell" v-if="approverForm.hasPrintBtn">
                  <p style="width:112px"></p>
                  <CCIT-TreeSelect :options="printTplList" v-model="approverForm.printId"
                    placeholder="请选择打印模板" lastLevel clearable></CCIT-TreeSelect>
                </div>
              </el-form-item>
    ...
    <el-dialog title="数据传递" :close-on-click-modal="false" :visible.sync="ruleVisible"
      class="CCIT-dialog CCIT-dialog_center rule-dialog" lock-scroll append-to-body width='700px'>
...
const getDataType = (data) => {
  if (!data.__config__ || !data.__config__.ccitKey) return ''
  const ccitKey = data.__config__.ccitKey
  if (['numInput', 'date', 'rate', 'slider'].includes(ccitKey)) {
    return 'number'
  } else if (['checkbox', 'uploadFz', 'uploadImg', 'cascader', 'comSelect', 'address'].includes(ccitKey)) {
    return 'array'
  } else if (['select', 'depSelect', 'posSelect', 'userSelect', 'treeSelect'].includes(ccitKey)) {
...
},
{
  ...
  __vModel__: 'ccitFlowOperatorUserId',
},
{
  __config__: {
    label: '当前操作用户名',
  ...
  __vModel__: 'ccitFlowOperatorUserName',
}]
export default {
  ...
  methods: {
        ...
        const loop = (data, parent) => {
          ...
          if (data.__vModel__) {
            ...
            res.push({
              ...
              requiredDisabled: requiredDisabled(data.__config__.ccitKey) || data.__config__.required,
              ccitKey: data.__config__.ccitKey,
    ...
    */
    ...
    fieldNameChange(val, item, i) {
      ...
      item.ccitKey = obj.__config__.ccitKey
      item = { ...item, ...obj }
    ...
    onConditionDateChange(val, item) {
      ...
      item.filedLabel = this.ccit.toDate(val, format)
    },
