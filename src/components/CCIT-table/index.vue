<template>
  <div class="tableContainer">
    <el-table v-if="!refreshTable" :data="data" ref="CCITTable" class="CCIT-common-table"
      ...
      <el-table-column prop="index" type="index" width="50" label="序号" v-if="hasNO"
        align="center" />
      <ccit-table-column :columns="columns" :columnList="columnList" v-if="customColumn" />
      <template v-else>
        <slot />
      </template>
...
<script>
import CcitTableColumn from './Column'
import ColumnSettings from './ColumnSettings'
export default {
  name: 'CCIT-table',
  components: { CcitTableColumn, ColumnSettings },
  props: {
    data: {
      type: Array,
      default: () => []
  ...
  methods: {
    ...
    getColumns() {
      ...
      const cacheList = this.ccit.storageGet(this.menuId)
      if (!cacheList) {
    ...
    doLayout() {
      setTimeout(() => {
        this.$nextTick(() => {
          this.$refs && this.$refs.CCITTable && this.$refs.CCITTable.doLayout()
        })
      }, 50)
    },
    setColumn(list) {
      // this.loading = true
      this.ccit.storageSet({ [this.menuId]: list })
