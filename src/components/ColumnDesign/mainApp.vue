    ...
    <div class="right-board">
      ...
      <div class="field-box">
        <div class="columnList" v-show="currentTab==='sort'">
          <el-table :data="sortOptions" class="CCIT-common-table" height="100%"
        ...
        <div class="columnList" v-show="currentTab==='search'">
          <el-table :data="searchOptions" class="CCIT-common-table" height="100%"
        ...
        <div class="columnList" v-show="currentTab==='field'">
          <el-table :data="columnOptions" class="CCIT-common-table" height="100%"
...
const getSearchType = item => {
  const ccitKey = item.__config__.ccitKey
  // 等于-1  模糊-2  范围-3
  const fuzzyList = [...useInputList]
  const RangeList = [...useDateList, 'time', 'date', 'numInput', 'calculate']
  if (RangeList.includes(ccitKey)) return 3
  if (fuzzyList.includes(ccitKey)) return 2
  return 1
}
  ...
  created() {
    ...
    const loop = (data, parent) => {
      ...
      if (data.__config__ && data.__config__.ccitKey) {
        ...
        if (data.__config__.layout === "colFormItem" && data.__vModel__ && data.__vModel__.indexOf('_ccit_') < 0 && visibility) {
          list1.push(data)
        }
      }
    }
    ...
    let searchOptions = list.filter(o => noSearchList.indexOf(o.__config__.ccitKey) < 0)
    let sortOptions = list1.filter(o => noColumnShowList.indexOf(o.__config__.ccitKey) < 0)
