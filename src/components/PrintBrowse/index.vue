<template>
  <el-dialog v-bind="$attrs" :close-on-click-modal="false" :modal-append-to-body="false"
    v-on="$listeners" @open="onOpen" fullscreen lock-scroll class="CCIT-full-dialog"
...
export default {
  ...
  methods: {
    ...
    replaceSysValue() {
      ...
      const systemPrintTime = this.ccit.toDate(new Date())
      ...
      if (recordList.length) {
        ...
        for (let i = 0; i < recordList.length; i++) {
          ...
          let desc = (record.userName || record.handleId) + '于' + this.ccit.toDate(record.handleTime) + (record.handleStatus == 1 ? '审批通过' : '审批拒绝') + (record.handleOpinion ? '，审批意见：' + record.handleOpinion : '')
