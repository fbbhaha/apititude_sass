          ...
          <template v-if="activeData.__config__">
            <template v-if="$store.getters.hasTable">
              <template v-if="activeData.__config__.ccitKey==='table'">
                <el-form-item
                  v-if="activeData.__vModel__!==undefined && !noVModelList.includes(activeData.__config__.ccitKey)"
                  label="控件字段">
            ...
            <PopupSelect v-if="activeData.__config__.ccitKey==='popupSelect'"
              :active-data="activeData" />
            <PopupAttr v-if="activeData.__config__.ccitKey==='popupAttr'" :active-data="activeData"
              ref="popupAttr" />
            <Tab v-if="activeData.__config__.ccitKey==='tab'" :active-data="activeData" />
            <Collapse v-if="activeData.__config__.ccitKey==='collapse'" :active-data="activeData" />
            <template v-if="activeData.__config__.ccitKey==='barcode'">
            ...
            <template v-if="activeData.__config__.ccitKey==='qrcode'">
              <el-form-item label="控件标题">
                <el-input v-model="activeData.__config__.label" placeholder="请输入控件标题" />
              </el-form-item>
            ...
            <template v-if="activeData.__config__.ccitKey==='button'">
            ...
            <template v-if="isSystem">
              ...
              <el-form-item label="选择模板" v-if="activeData.__config__.ccitKey==='billRule'">
                <BillRule v-model="activeData.__config__.rule" placeholder="选择模板" />
              </el-form-item>
              <el-form-item label="显示内容" v-if="activeData.__config__.ccitKey==='currOrganize'">
          ...
          <template v-if="activeData.__config__">
            <template v-if="!activeData.__config__.isSubTable">
              ...
              <el-form-item label="标题宽度"
                v-if="!['divider','CCITText','button','table','groupTitle ','card','row','tab','collapse'].includes(activeData.__config__.ccitKey)">
          ...
          <template v-if="modelType!=3 && modelType!=6 && webType!=3">
            ...
            <template v-if="webType==2">
              ...
              <el-form-item label="" v-if="formConf.hasPrintBtn">
                <CCIT-TreeSelect :options="printTplList" v-model="formConf.printId"
...
export default {
  ...
  watch: {
    ...
    activeData(val) {
      ...
      if (val.__config__.ccitKey === 'popupAttr') {
        this.$nextTick(() => {
      ...
      if (!val.__config__.tableName && val.__config__.ccitKey !== 'table') {
  ...
  created() {
    ...
    if (!this.activeData.__config__.visibility) this.$set(this.activeData.__config__, "visibility", ["pc", "app"])
    if (!this.activeData.__config__.tableName && this.activeData.__config__.ccitKey !== 'table') {
  ...
  methods: {
    ...
    fieldChange1(val) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if (config.ccitKey === 'table' && config.tableName === this.activeData.__config__.relationTable) {
    ...
    setDefaultOptions() {
      ...
      if (this.activeData.__vModel__ === undefined || this.activeData.__config__.ccitKey === 'table') return
      if (!this.activeData.__config__.tableName || this.activeData.__config__.tableName === this.mainTable) {
        ...
        if (!list.length) {
          ...
          let options = item.fields.map(o => ({
            ...o,
            realField: 'ccit_' + this.activeData.__config__.tableName + '_ccit_' + o.field,
          }))
    ...
    onTableNameChange(tableName) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          const e = list[i]
          const config = e.__config__
          if (config.ccitKey === 'table' && config.tableName === tableName) {
            boo = true
            break
          }
          if (config && config.ccitKey != 'table' && config.children && Array.isArray(config.children)) {
