...
<template>
  <div class="container">
    ...
    <json-drawer
      ...
      @refresh="refreshJson"
    />
      <Preview :visible.sync="previewVisible" :showType="showType" :form-data="formData" />
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    if (this.modelType == 1 || this.modelType == 6)
      this.leftComponents[1].list = [
        ...this.leftComponents[1].list,
        ...onlinePeculiarList,
        ...webPeculiarList,
      ];
    if (this.webType != 2 || this.modelType == 3 || this.modelType == 6)
      this.formConf.popupType = "fullScreen";
  },
  mounted() {
  ...
  methods: {
        ...
        const loop = (list) => {
          for (let i = 0; i < list.length; i++) {
            ...
            if (
              ...
              !noVModelList.includes(config.ccitKey) &&
              !e.__vModel__
            ) {
    ...
    shouldClone(to, from, target, event, conf) {
      ...
      const isRowContainer =
        conf.__config__.cmpType === "common" &&
        conf.__config__.rowType === "layout";
    ...
    onEnd(obj) {
      ...
      if (obj.to.className.indexOf("table") > -1) {
        ...
        if (this.$store.getters.hasTable) {
          this.$set(
            this.activeItem.__config__,
            "relationTable",
            this.activeTableItem.__config__.tableName
          );
    ...
    createIdAndKey(item) {
      ...
      if (config.layout === "colFormItem") {
        if (!this.$store.getters.hasTable) {
          // 分割线和按钮不加vModel
          if (noVModelList.indexOf(config.ccitKey) < 0) {
            item.__vModel__ = this.toggleVmodelCase(
              `${config.ccitKey}Field${this.idGlobal}`
            );
          }
        } else {
          if (noVModelList.indexOf(config.ccitKey) < 0) {
    ...
    toggleVmodelCase(str) {
      ...
      if (
        dbType.toLowerCase() === "Oracle".toLowerCase() ||
        dbType.toLowerCase() === "DM8".toLowerCase()
      ) {
        return str.toUpperCase();
      }
      if (
        dbType.toLowerCase() === "PostgreSQL".toLowerCase() ||
        dbType.toLowerCase() === "KingBaseES".toLowerCase()
      ) {
    ...
    relationChange(vmodel) {
      const loop = (list) => {
        for (let i = 0; i < list.length; i++) {
          ...
          if (
            list[i].relationField &&
            list[i].relationField === list[i].__vModel__
          ) {
            list[i].showField = "";
          }
          if (config && config.children && Array.isArray(config.children)) {
