...
const layouts = {
  colFormItem(h, element, index, parent) {
    ...
    if (config.ccitKey === 'barcode') {
      return (
        <el-col span={24} class={className}
          nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
          <el-form-item label-width={labelWidth}
            label={config.showLabel ? config.label : ''} required={config.required}>
            <ccit-barcode format={element.format} lineColor={element.lineColor} background={element.background} width={element.width} height={element.height} staticText={element.staticText}></ccit-barcode>
          </el-form-item>
          {components.itemBtns.apply(this, arguments)}
        </el-col>
      )
    }
    if (config.ccitKey === 'qrcode') {
      return (
        <el-col span={24} class={className}
          nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
          <el-form-item label-width={labelWidth}
            label={config.showLabel ? config.label : ''} required={config.required}>
            <ccit-qrcode size={element.size} colorLight={element.colorLight} colorDark={element.colorDark} staticText={element.staticText}></ccit-qrcode>
          </el-form-item>
          {components.itemBtns.apply(this, arguments)}
        </el-col>
      )
    }
    if (config.ccitKey === 'button') {
      return (
        <el-col span={24} class={className}
          nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
          <el-form-item label-width="0">
            <ccit-button align={element.align} buttonText={element.buttonText} type={element.type} disabled={element.disabled}></ccit-button>
          </el-form-item>
          {components.itemBtns.apply(this, arguments)}
        </el-col>
      )
  ...
  rowFormItem(h, element, index, parent) {
    ...
    const className = this.activeId === element.__config__.formId
      ...
      : 'drawing-row-item'
    if (element.__config__.ccitKey === 'tab') {
      return (
        <el-col span={24}>
    ...
    if (element.__config__.ccitKey === 'collapse') {
      return (
        <el-col span={24}>
          <el-row gutter={element.__config__.gutter} class={className}
            nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
