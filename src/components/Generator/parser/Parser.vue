...
const layouts = {
  colFormItem(h, scheme) {
    ...
    const Item = config.ccitKey === 'cascader'
  ...
  rowFormItem(h, scheme) {
    ...
    if (scheme.__config__.ccitKey === 'tab') {
      return (
    ...
    if (scheme.__config__.ccitKey === 'collapse') {
      return (
...
function buildListeners(scheme) {
  ...
  if (scheme.on) {
    ...
    Object.keys(scheme.on).forEach(key => {
      ...
      const func = this.ccit.getScriptFunc.call(this, str);
      if (!func) return
      listeners[key] = params => {
        if (key === 'change') {
          let data = ''
          if (['select', 'radio', 'checkbox'].includes(config.ccitKey)) {
            const options = scheme.__slot__.options
            if (scheme.multiple || config.ccitKey === 'checkbox') {
              let _data = []
          ...
          } else if (config.ccitKey === 'numInput') {
            data = params[0]
          } else {
  ...
  methods: {
    ...
    buildRules(componentList, rules) {
      componentList.forEach(cur => {
        ...
        if (config.children && config.ccitKey !== 'table') this.buildRules(config.children, rules)
      })
    },
    onLoad(formConfCopy) {
      if (!formConfCopy || !formConfCopy.funcs || !formConfCopy.funcs.onLoad) return
      const onLoadFunc = this.ccit.getScriptFunc.call(this, formConfCopy.funcs.onLoad)
          ...
          if (item.__vModel__ && item.__vModel__ === prop) {
            switch (field) {
              case 'disabled':
                ...
                break;
              case 'options':
                if (dyOptionsList.indexOf(item.__config__.ccitKey) > -1) {
    ...
    beforeSubmit() {
      ...
      const func = this.ccit.getScriptFunc.call(this, this.formConfCopy.funcs.beforeSubmit)
    ...
    afterSubmit() {
      ...
      const func = this.ccit.getScriptFunc.call(this, this.formConfCopy.funcs.afterSubmit)
      if (!func) return
      func(this.parameter)
    },
    submitForm() {
    ...
    onCascaderChange(data, on) {
      ...
      const func = this.ccit.getScriptFunc.call(this, on.change)
      if (!func) return
      func.call(this, { data, ...this.parameter })
    },
    onCascaderBlur(data, on) {
      if (!on || !on.blur) return
      const func = this.ccit.getScriptFunc.call(this, on.blur)
      if (!func) return
      func.call(this, { data, ...this.parameter })
    }
  },
