<template>
  <div class="ccit-table-box" :class="[config.__config__.type]">
    <div class="CCIT-common-title" v-if="config.__config__.showTitle && config.__config__.label">
      <h2>{{ config.__config__.label }}</h2>
    </div>
    <el-table :data="tableFormData" class="CCIT-common-table" @cell-click="focusInput"
      ...
      <template v-for="(head, cindex) in tableData">
        <el-table-column :key="head.__config__.formId" :min-width="head['min-width']"
          ...
          <template slot-scope="scope">
            ...
            <template v-if="['select', 'checkbox','radio'].includes(head.__config__.ccitKey)">
  ...
  methods: {
    ...
    setTableFormData(prop, value) {
      ...
      for (let i = 0; i < activeRow.length; i++) {
        ...
        if (activeRow[i].__vModel__.indexOf('_ccitRelation_') >= 0) {
          vModel = activeRow[i].__vModel__.substring(0, activeRow[i].__vModel__.indexOf('_ccitRelation_'))
        }
        if (vModel === prop) {
    ...
    getTableFieldOptions(prop) {
      ...
      for (let i = 0; i < this.tableData.length; i++) {
        if (this.tableData[i].__vModel__ === prop) {
          ...
          let isTreeSelect = item.__config__.ccitKey === 'treeSelect' || item.__config__.ccitKey === 'cascader'
    ...
    onFormBlur(rowIndex, colIndex, tag) {
      ...
      if (data && data.on && data.on.blur) {
        const func = this.ccit.getScriptFunc.call(this, data.on.blur)
        if (!func) return
        func.call(this, {
          data: null,
          ...this.parameter
    ...
    onFormDataChange(rowIndex, colIndex, tag, params) {
      ...
      if (data && data.on && data.on.change) {
        const func = this.ccit.getScriptFunc.call(this, data.on.change)
        if (!func) return
        let value = ''
        if (['select', 'radio', 'checkbox'].includes(data.ccitKey)) {
          const options = data.options
          if (data.config.multiple || data.ccitKey === 'checkbox') {
            let _value = []
    ...
    getConfById(formId, rowIndex) {
      ...
      if (['relationForm', 'popupSelect'].includes(itemConfig.ccitKey)) {
        newObj['field'] = item.__vModel__ + '_ccitRelation_' + rowIndex
      }
      if (['relationFormAttr', 'popupAttr'].includes(itemConfig.ccitKey)) {
        let prop = newObj['relationField'].split('_ccitTable_')[0]
        newObj['relationField'] = prop + '_ccitRelation_' + rowIndex
      }
      return newObj
    },
    /**
    ...
    getEmptyRow(val, rowIndex) {
      return this.tableData.map((t, index) => {
        let res = {
          ...
          regErrorText: '',
          on: t.on || {},
          ccitKey: t.__config__.ccitKey,
          __vModel__: ['relationForm', 'popupSelect'].includes(t.__config__.ccitKey) ? t.__vModel__ + '_ccitRelation_' + rowIndex : t.__vModel__,
    ...
    getTableValue() {
      return this.tableFormData.map(row => row.reduce((p, c) => {
        ...
        if (c.__vModel__ && c.__vModel__.indexOf('_ccitRelation_') >= 0) {
          str = c.__vModel__.substring(0, c.__vModel__.indexOf('_ccitRelation_'))
        }
        p[str] = c.value
