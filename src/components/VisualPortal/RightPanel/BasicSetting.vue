<template>
  <div v-if="activeData">
    ...
    <template v-if="!activeData.usePortalStyle">
      ...
      <el-form-item v-if="compConfig.themeColor !== undefined" label="换肤">
        ...
        <div class="color-list">
          <template v-for="(item, index) in colorList">
            ...
            >
              <i
                class="el-icon-check"
                v-if="item.className === activeData.themeColor"
              ></i>
            </el-tag>
          </template>
        </div>
...
export default {
  ...
  methods: {
    changeColor(item) {
      if (item.className === this.activeData.themeColor) {
        this.activeData.themeColor = "";
      } else {
        this.activeData.themeColor = item.className;
      }
    },
    cardStyleChange(e) {
      if (e === "default") {
        ...
        this.activeData.themeColor = "blue";
      }
    },
    editStyle() {
    ...
    initBarChart(data, dataInterface) {
      ...
      if (this.activeData.option.xAxis[0].type == "category") {
        this.activeData.option.xAxis[0].data = data.categories;
      } else {
        this.activeData.option.yAxis[0].data = data.categories;
      }
      //设置数据
      this.activeData.option.series = [];
      this.activeData.option.legend.data = [];
