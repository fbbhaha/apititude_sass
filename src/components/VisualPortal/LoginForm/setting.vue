<template>
  <div>
    ...
    <el-form-item label="登录logo" label-width="90px">
      <CCIT-UploadFz
        v-model="fileList"
        :limit="1"
        buttonText="点击上传"
    ...
    <el-form-item label="记住密码" label-width="90px">
      <el-switch v-model="activeData.useRemember"></el-switch>
    </el-form-item>
    <div v-for="(item, index) in activeData.list" :key="index">
      <template v-if="index !== 2 || (needVerification && verificationType == 0)">
        <el-divider>{{ item.fullName }}</el-divider>
        <el-form-item v-if="item.show != undefined" label="显示" label-width="90px">
          <el-switch v-model="item.show"></el-switch>
        </el-form-item>
        <el-form-item v-if="item.focus !== undefined" label="激活效果" label-width="90px">
        ...
        <template v-if="item.iconType === 'icon'">
          <el-form-item label-width="90px" :label="item.focus ? '未激活图标' : '选择图标'">
            <el-input v-model="item.originIcon" placeholder="请输入图标名称">
              <el-button
                slot="append"
                @click="openIconsDialog(index, 'originIcon')"
              >
                选择</el-button
              >
            </el-input>
          </el-form-item>
          <el-form-item label-width="90px" v-if="item.focus" label="激活时图标">
            <el-input v-model="item.activeIcon" placeholder="请输入图标名称">
              <el-button
                slot="append"
                @click="openIconsDialog(index, 'activeIcon')"
              >
                选择</el-button
              >
            </el-input>
        ...
        <el-form-item
          v-if="item.originColor"
          :label="index === 3 ? '背景色' : '边框颜色'"
          label-width="90px"
        >
          <el-row>
            <el-col style="display: flex" :span="item.focus ? 12 : 24">
              {{ item.focus ? "未激活" : "" }}
            ...
            <el-col
              style="display: flex"
              v-if="item.focus && item.shadowColor"
              :span="11"
            >
              阴影
              <el-color-picker v-model="item.shadowColor"></el-color-picker
            ></el-col>
          </el-row>
        </el-form-item>
        <el-form-item v-if="item.placeholder" label="占位文本" label-width="90px">
          <el-input v-model="item.placeholder" placeholder="请输入占位文本">
          </el-input>
        </el-form-item>
        <el-form-item v-if="item.text" label="文本" label-width="90px">
          <el-input v-model="item.text" placeholder="请输入文本"> </el-input>
        </el-form-item>
        <el-form-item v-if="item.css" label="样式" label-width="90px">
