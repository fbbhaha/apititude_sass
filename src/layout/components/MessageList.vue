<template>
  <div>
    <el-drawer title="站内消息" :visible.sync="drawer" direction="rtl" size="280px"
      class="CCIT-messageList CCIT-common-drawer" :before-close="handleClose">
      <div class="tool">
        <el-link :underline="false" @click.native="readAll">全部已读</el-link>
        <el-link :underline="false" @click.native="gotoCenter">消息中心</el-link>
      </div>
      <div class="CCIT-messageList-box" v-loading="loading && listQuery.currentPage==1"
        ref="messageListBody">
        <div v-if="list.length">
          <div v-for="(item,i) in list" :key="i" class="CCIT-messageList-item"
            @click="readInfo(item)" :title="item.title">
            <el-badge is-dot :hidden="item.isRead=='1'" type="warning">
              <i class="icon-ym icon-ym-xitong CCIT-messageList-item-icon"></i>
            </el-badge>
    ...
    <el-dialog title="查看消息" :close-on-click-modal="false" :visible.sync="visible"
      class="CCIT-dialog CCIT-dialog_center CCIT-dialog-notice" lock-scroll width="80%">
      <div class="notice-wrapper" v-loading="loading">
        ...
        <div class="file-list" v-if="files.length">
          <CCIT-UploadFz v-model="files" disabled detailed />
        </div>
      </div>
